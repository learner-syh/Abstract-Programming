...
Set<GridLocation> stackToSet(Stack<GridLocation> path){
    Set<GridLocation> usedLocation;
    while(!path.isEmpty()){
        usedLocation.add(path.pop());
    }
    return usedLocation;
}
...
Set<GridLocation> generateValidMoves(Grid<bool>& maze, GridLocation cur) {
    ...
    potentialNeighbors.add(GridLocation(cur.row-1, cur.col));
    potentialNeighbors.add(GridLocation(cur.row, cur.col-1));
    potentialNeighbors.add(GridLocation(cur.row+1, cur.col));
    potentialNeighbors.add(GridLocation(cur.row, cur.col+1));
    //判断条件，移除不满足条件的点
    for (GridLocation elem : potentialNeighbors) {
        if(maze.inBounds(elem) && maze.get(elem) )
            neighbors.add(elem);
    }
    return neighbors;
}
...
void validatePath(Grid<bool>& maze, Stack<GridLocation> path) {
    ...
    /* TODO: Fill in the remainder of this function. */
    Set<GridLocation> existingLocation;
    GridLocation curLocation;
    while(path.size() > 1){
        curLocation = path.pop();
        existingLocation.add(curLocation);
        //判断下一个位置是否正确
        if(!generateValidMoves(maze,curLocation).contains(path.peek())){
            error("Path does not correct");
        }
        //判断路径是否重复
        if(existingLocation.contains(path.peek())){
            error("The path is repeated");
        }
    }
...
Stack<GridLocation> solveMaze(Grid<bool>& maze) {
    ...
    while(!todo.isEmpty()){
        path = todo.dequeue();
        MazeGraphics::highlightPath(path,"blue",1);
        if(exit == path.peek()){
            return path;
        }
        usedLocation = stackToSet(path);
        for(GridLocation elem : generateValidMoves(maze,path.peek())){
...
STUDENT_TEST("generateValidMoves on corner of 3X3 grid with aLL walls"){
...
STUDENT_TEST("generateValidMoves on corner of 3X3 grid with corner"){
    Grid<bool> maze = {{true, true, true},
                       {true, true, true},
                       {true, true, true}};
