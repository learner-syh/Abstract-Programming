...
Stack<string> findWordLadder(string start, string end) {
    //准备与一个记事本
    Queue<Stack<string>> todo;
    Stack<string> init = {start};
    todo.enqueue(init);
    //todolist不为空持续寻找
    while(!todo.isEmpty()){
        //取出最先处理的任务
        Stack<string>curLadder = todo.dequeue();
        //查询是否为最终结果
        string word = curLadder.peek();
        //这里是输出所有可能路径的代码，需要的话，解开注释，并注释掉下一段代码
//        if(word == end){
//            Stack<string>copy1 =curLadder;
//            while (!copy1.isEmpty()) {
//                cout << copy1.pop() << endl;
//            }
//            cout<<endl;
//        }
        ...
        auto visited = visitedWords(curLadder);
        //找到所有改变一个字母的单词集合
        Set<string> next = nextWords(word);
        for(const string &item : next){
            if(!visited.contains(item)){
                Stack<string> copy = curLadder;
                copy.push(item);
                todo.enqueue(copy);
            }
        }
