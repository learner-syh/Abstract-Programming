 ...
 */
const string kYourName = "HAO";

/* Change this constant to the path out of your labyrinth. */
const string kPathOutOfNormalMaze = "SESENENWESWSSENSWNWSW";
...
bool isPathToFreedom(MazeCell* start, string moves, Set<string> needs) {
    /* TODO: Your code here */
    MazeCell *nowCell = start;
    for ( string::iterator it=moves.begin(); it!=moves.end(); ++it){
        if(needs.contains(nowCell->contents) ){
            needs.remove(nowCell->contents);
        }
        if(needs.isEmpty()){
            return true;
        }
        if(*it == 'E' ){
            nowCell = nowCell ->east;
            if(nowCell == nullptr){
                return false;
            }
        }else if(*it == 'S' ){
            nowCell = nowCell ->south;
            if(nowCell == nullptr){
                return false;
            }
        }else if(*it == 'W' ){
            nowCell = nowCell ->west;
            if(nowCell == nullptr){
                return false;
            }
        }else if(*it == 'N' ){
            nowCell = nowCell ->north;
            if(nowCell == nullptr){
                return false;
            }
        }else{
            error("wrong moving direction!");
        }
    }
    if(needs.contains(nowCell->contents) ){
        needs.remove(nowCell->contents);
